...
def button3_command():
    global input_var, input_box, exit_button, input_label_created, s
...
def send_input():
    global input_label, input_label_created
    input_text = input_var.get()
    if not input_label_created:
        input_label = tk.Label(root, text=input_text)
        input_label.pack()
        input_label_created = True
    else:
        input_label.config(text=input_label.cget("text") + '\n' + input_text)
